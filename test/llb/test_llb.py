        ...
        try:
            ...
            if self.skill_integrator and result.retrieved_skills:
                ...
                if result.success:
                    ...
                    actual_error = f"LLB task failed with exit code {result.exit_code}" if hasattr(result, 'exit_code') else "LLB task failed"
                    for skill_name in result.retrieved_skills:
                        lqrl_result = self.skill_integrator.process_task_failure_and_update(
                            skill_name=skill_name,
                            actual_error=actual_error,
                            task_context=instruction[:500] if instruction else "",
                        )
